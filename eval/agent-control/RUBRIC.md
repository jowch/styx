...
| D4 | **Glass hygiene** | Hardcoded ports + tab spam; child `newTab` | Eventual correct URL; extra tabs or retries | **Parent:** local host URL / remote remember→probe→ask; tab reuse; no `newTab`. **Child:** empty list + view-not-found accepted; no `newTab` |
...
3. **Parent re-grades** from the tool trace (do not trust self-score alone). Prefer evidence:
   - `pluto_session_status` / host `pluto_url` (local) / Glass URL ladder remember→probe→ask (Remote SSH)
