...
| C3 | **URL resolution** | Glass uses `pluto_url` from status; laptop handoff uses optional `client_url` when present — never invent remaps | Hardcodes `:1234` or invents client ports |
| C4 | **Tab reuse** | Reuses Agents Glass / `position: "active"`; no `newTab`/`new` by default | Tab spam after empty list or failed navigate |
