...
| C3 | **URL resolution** | Local Glass uses **exact** host from `pluto_url` (no `localhost`↔`127.0.0.1` rewrite); Remote SSH asks once per session for Ports forwarded port → `http://127.0.0.1:<forwarded>/` and keeps that host | Hardcodes `:1234`, invents remaps, skips the remote ask, or mixes loopback hosts mid-session |
