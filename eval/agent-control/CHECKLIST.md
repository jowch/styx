...
|---|--------|-----------------|-------------|
| C1 | **Skill routing** | Notebook intent → `pluto-session` then `pluto-workflow` / `pluto-semantics`; **styx-start** finishes Glass + Path B in the **same turn** (not print URL and stop); `styx-setup` only when install/Julia/MCP is broken | Raw MCP / shell `serve` without skills; `/styx-start` that prints `welcome_url=` and asks the user to open it |
...
| C3 | **URL resolution** | Local Glass uses **exact** host from `pluto_url` (no `localhost`↔`127.0.0.1` rewrite); Remote SSH: remember last-good iff `glass-views.json` `pluto_port` matches live host port → same-port Glass probe → ask Ports **only if** probe fails; keep that host | Hardcodes `:1234`, invents remaps, asks Ports when remember/probe would work, remote `curl` as Glass proof, or mixes loopback hosts mid-session |
