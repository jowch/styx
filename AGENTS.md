...
- **Eval harness** (`eval/`): Julia-only CI gate via `run_reference.jl --all --strict-trace` (SDK `run.ts` removed); D15 deferred validation via **`scripts/d15-validate-deferred.sh`** (Scenarios 0/C/D/E). PlutoMCP keeps optional `EvalLog.jl` hook only.
