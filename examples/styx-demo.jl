...
# ╔═╡ 8f3b2c1a-5d4e-4f6a-9b8c-7d6e5f4a3b2c
md"""
# Styx demo

Reactive [Pluto.jl](https://github.com/fonsp/Pluto.jl) notebook for trying **Styx** in Cursor.

1. Open this file in Glass (agent: *"open the Styx demo notebook"*).
2. Click **Run notebook code** if outputs are stale (safe preview).
3. Toggle **⌘⇧D** (Design Mode), click the plot cell below, and describe an edit in chat.

Drag the range slider — the plot updates live. Agent edits use the same cell structure Styx expects (`imports_cell` → `widget_cell` → `compute_cell`).
"""
