...
md"""
# Cell-seg demo (BBBC038 nuclei)

Thin Styx spike: **load a tiny BBBC038 subset → visualize → train a lightweight classifier**.

Not a research segmentation model — just enough pipeline to prove the notebook story for a recorded demo.

### Data & license
- **Source:** [BBBC038v1](https://bbbc.broadinstitute.org/BBBC038) — Kaggle 2018 Data Science Bowl nuclei (Broad Bioimage Benchmark Collection).
- **Download:** [`stage1_train.zip`](https://data.broadinstitute.org/bbbc/BBBC038/stage1_train.zip) (~83 MB, public HTTP; no Kaggle login).
- **This notebook** caches the zip under `~/.cache/styx-bbbc038-demo/` and extracts **N = 8** image folders (images + instance masks).
- **License / attribution:** BBBC terms; cite *"We used image set BBBC038v1, available from the Broad Bioimage Benchmark Collection [Caicedo et al., Nature Methods, 2019]."* (see Broad page for contributor credits). Dataset is redistributed by Broad for research/demo use.

Optional pre-fetch (same cache path):
...
begin
	ZIP_URL = "https://data.broadinstitute.org/bbbc/BBBC038/stage1_train.zip"
	N_IMAGES = 8
	CACHE_DIR = joinpath(homedir(), ".cache", "styx-bbbc038-demo")
	ZIP_PATH = joinpath(CACHE_DIR, "stage1_train.zip")
	SUBSET_DIR = joinpath(CACHE_DIR, "subset")
end

# ╔═╡ a1111111-1111-4111-8111-111111111104
begin
	function ensure_zip!(url=ZIP_URL, path=ZIP_PATH)
		mkpath(dirname(path))
		if isfile(path)
			return path
		end
		@info "Downloading BBBC038 stage1_train (~83 MB; once)" url path
	...
	function extract_subset!(zip_path::AbstractString, ids, out_dir::AbstractString)
		...
		try
			for f in z.files
				...
				parts[1] in wanted || continue
				endswith(f.name, '/') && continue
				dest = joinpath(out_dir, f.name)
	...
	function load_gray(path::AbstractString)
		Float64.(Gray.(load(path)))
	end

	function union_mask(mask_dir::AbstractString)
		files = filter(f -> endswith(f, ".png"), readdir(mask_dir; join=true))
		isempty(files) && error("no masks in $mask_dir")
		u = falses(size(load_gray(files[1])))
		for f in files
			u .|= (load_gray(f) .> 0.5)
		end
		return u, length(files)
	end

	function nucleus_features(img::AbstractMatrix, mask_dir::AbstractString)
		files = filter(f -> endswith(f, ".png"), readdir(mask_dir; join=true))
		feats = NamedTuple{(:area, :mean_int, :aspect), Tuple{Float64,Float64,Float64}}[]
		for f in files
			m = load_gray(f) .> 0.5
			area = count(m)
			area == 0 && continue
			mean_int = mean(@view img[m])
			coords = findall(m)
			ys = [i[1] for i in coords]
			xs = [i[2] for i in coords]
			h = maximum(ys) - minimum(ys) + 1
			w = maximum(xs) - minimum(xs) + 1
			push!(feats, (area=Float64(area), mean_int=Float64(mean_int), aspect=Float64(h / max(w, 1))))
		end
		return feats
	end
end
...
begin
	t_fetch = time()
	ensure_zip!()
	image_ids = list_image_ids(ZIP_PATH)[1:N_IMAGES]
	extract_subset!(ZIP_PATH, image_ids, SUBSET_DIR)
	fetch_secs = round(time() - t_fetch; digits=2)
	md"""
	**Fetch / extract:** $(length(image_ids)) images → `$SUBSET_DIR` in **$(fetch_secs)s**
	(zip cached at `$ZIP_PATH`).
	"""
end

# ╔═╡ a1111111-1111-4111-8111-111111111106
begin
	samples = map(image_ids) do id
		img_path = joinpath(SUBSET_DIR, id, "images", id * ".png")
		mask_dir = joinpath(SUBSET_DIR, id, "masks")
		img = load_gray(img_path)
		overlay, n_masks = union_mask(mask_dir)
		feats = nucleus_features(img, mask_dir)
		(; id, img, overlay, n_masks, feats)
	end
	n_nuclei = sum(length(s.feats) for s in samples)
	md"""
	**Loaded** $(length(samples)) images · **$(n_nuclei)** nuclei (instance masks).
	"""
end
...
Pick an image. Left: grayscale nuclei. Right: union of instance masks (demo overlay — not a model prediction).
"""

# ╔═╡ a1111111-1111-4111-8111-111111111108
@bind sample_idx Slider(1:length(samples); default=1, show_value=true)

# ╔═╡ a1111111-1111-4111-8111-111111111109
let
	s = samples[sample_idx]
	p1 = heatmap(s.img; yflip=true, aspect_ratio=1, color=:grays, axis=false,
		title="image $(s.id[1:8])…")
	p2 = heatmap(Float64.(s.overlay); yflip=true, aspect_ratio=1, color=:grays, axis=false,
		title="$(s.n_masks) masks")
	plot(p1, p2; layout=(1, 2), size=(780, 360))
end
...
**Object-level** toy task on nuclei from the subset:

- **Label:** bright vs dim (`mean intensity` ≷ median across nuclei)
- **Features:** `area`, bounding-box `aspect` (intensity is *not* a feature — that would leak the label)
- **Model:** `DecisionTree` (CPU, seconds)
- **Split:** 70% train / 30% test, fixed seed

Illustrative only — proves train → score plumbing for the demo.
"""

# ╔═╡ a1111111-1111-4111-8111-11111111110b
begin
	t_ml = time()
	all_feats = vcat((s.feats for s in samples)...)
	areas = [f.area for f in all_feats]
	ints = [f.mean_int for f in all_feats]
	aspects = [f.aspect for f in all_feats]
	intensity_threshold = median(ints)
	y = Int.(ints .> intensity_threshold)  # 1 = bright, 0 = dim
	X = hcat(areas, aspects)

	Random.seed!(42)
	n = size(X, 1)
	perm = randperm(n)
	n_test = max(1, round(Int, 0.3 * n))
	test_idx = perm[1:n_test]
	train_idx = perm[(n_test + 1):end]

	model = build_tree(y[train_idx], X[train_idx, :])
	yhat = apply_tree(model, X[test_idx, :])
	accuracy = mean(yhat .== y[test_idx])
	ml_secs = round(time() - t_ml; digits=2)

	md"""
	| | |
	|---|---|
	| Nuclei | $(n) |
	| Train / test | $(length(train_idx)) / $(length(test_idx)) |
	| Intensity threshold (median) | $(round(intensity_threshold; digits=4)) |
	| Test accuracy | **$(round(accuracy; digits=3))** |
	| Fit + score | $(ml_secs)s |
	"""
end

# ╔═╡ a1111111-1111-4111-8111-11111111110c
begin
	scatter(
		areas, aspects;
		zcolor=ints,
		xlabel="area (px)",
		ylabel="bbox aspect",
		title="Nuclei features (color = mean intensity)",
		legend=false,
		markersize=3,
		size=(640, 420),
	)
...
# ╔═╡ a1111111-1111-4111-8111-11111111110d
md"""
### Demo notes
- First open resolves Pluto packages + may download the 83 MB zip once; later runs use `~/.cache/styx-bbbc038-demo/`.
- Bump `N_IMAGES` if you want a denser slide — keep it small for recording.
- Sibling toy notebook: [`styx-demo.jl`](./styx-demo.jl) (sinc + slider).
"""
...
manifest_format = "2.1"
project_hash = "3d13fc78f086647068fec1eb74bb089438be12bb"
