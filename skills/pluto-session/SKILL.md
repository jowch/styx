...
| Rewrite `127.0.0.1` ↔ `localhost` mid-session | Same port ≠ same origin — cookie fails (“Not yet authenticated”). One host per session: exact host from `pluto_url` (local) or the Ports URL (remote) — [glass-navigation.md](reference/glass-navigation.md#one-host-per-session-localhost--127001) |
