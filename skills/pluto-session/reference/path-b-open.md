...
2. Open **landing** in Agents Glass — [glass-navigation.md](glass-navigation.md): local → `browser_navigate({ url: status.pluto_url })`; Remote SSH → remember → same-port Glass probe → ask Ports only if needed, then navigate that origin
