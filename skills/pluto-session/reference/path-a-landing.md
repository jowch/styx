...
2. Open **Pluto landing page** in Agents Glass — see [glass-navigation.md](glass-navigation.md) (`cursor-ide-browser` → `browser_navigate({ url })` with **no** `position` / `newTab`; local uses host `pluto_url`, Remote SSH asks for Ports forwarded port first)
